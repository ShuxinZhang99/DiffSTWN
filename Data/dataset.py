...
class CleanDataset():
    ...
    def read_data(self):
        ...
        # 根据文件后缀自动选择加载方式
            if isinstance(file_path, str):
                ...
                if ext == ".npy":
                    ...
                    # 使用 pandas 读取 csv，若没有表头可设置 header=None
                    # values 会直接转换为 numpy 数组
                    return pd.read_csv(file_path).values
                else:
                    raise ValueError(
                        f"不支持的文件格式: {ext}，仅支持 .npy 和 .csv 文件！"
                    )
            elif isinstance(file_path, np.ndarray):
                return file_path
            else:
                raise TypeError(f"不支持的数据类型: {type(file_path)}")

        # 自动识别并加载数据
        data = _load_file(self.feature_file)
        real_data = _load_file(self.real_file)
        ...
        if self.external_file is not None:
            ext_data = _load_file(self.external_file)
            ...
            external_feature = ext_data.astype(np.int32)
        else:
