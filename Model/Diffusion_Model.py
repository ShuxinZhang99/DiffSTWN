...
from ZeroInflated_NB_Loss import *
from STWN import *
...
class DiffWave(nn.Module):
    ...
    def __init__(self, N, sample_steps, sample_strategy, origin_graph, destination_graph, d_model, beta_schedule, con_time_length, con_pre_len,
                 num_nodes, num_stations, num_ext):
        ...
        # 1. Denoising Network
        # =====================
        self.eps_model_3 = EpsilonTheta(node_num=num_nodes, station_num=num_stations, ext_num=num_ext, target_len=con_pre_len,
                                        cond_length=con_time_length, o_graph=origin_graph, d_graph=destination_graph,
                                        d_model=d_model)
    ...
    def estimating_real_time_od(self, unfinished_inflow, long_term_rate, short_term_rate, finished_od):
        
        unfinished_inflow = unfinished_inflow.to(self.device)
        long_term_rate = long_term_rate.to(self.device)
        short_term_rate = short_term_rate.to(self.device)
        finished_od = finished_od.to(self.device)

        completed_real_od = self.od_estimator(unfinished_inflow, long_term_rate, short_term_rate, finished_od)  # [B, N*N, T_h]

        return completed_real_od
    ...
    def evaluate(self, input, ext_inputs, n_samples):
        ...
        n_params, p, pi = self.eps_model_3.zinb_model(x_masked[:, :, :, T_h:])
