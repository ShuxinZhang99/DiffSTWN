...
class ST_Decomposition(nn.Module):
    def __init__(self, moving_kernel_size, d_model, num_ext_factors, con_time_length, con_pre_len, num_nodes, num_stations, origin_graph, destination_graph, num_layer):  # d_adj_mx, max_diffusion_step,
        ...
        # external_modeling
        self.ext_embedding = nn.Embedding(num_embeddings=10, embedding_dim=d_model)
        ...
        self.st_to_dmodel = nn.Linear(1, self.d_model)
        # 3. 时空特征与外部特征交叉融合模块
    ...
    def forward(self, inputs, ext_inputs, o_adj=None, d_adj=None):
        ...
        ext_inputs = self.ext_embedding(ext_inputs)  # (B, N_ext, T, d_model)
        SE = self.ext_encoder(ext_inputs).unsqueeze(2).repeat(1, 1, V, 1)  # (B, T, V, d_model)
        ST_proj = self.st_to_dmodel(ST_raw).permute(0, 3, 2, 1)  # (B, T, V, d_model)
