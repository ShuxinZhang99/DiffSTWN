...
class ExternalEncoder(nn.Module):
    def __init__(self, num_ext_factors, d_model):
        ...
        self.ext_attn = nn.MultiheadAttention(embed_dim=d_model * num_ext_factors, num_heads=4, batch_first=True)
...
class ST_SE_CrossFusion(nn.Module):
    ...
    def forward(self, ST, SE):
        ...
        A_s, _ = self.attn_s(query=ST, key=ST, value=ST)  # (B, T, d_model)

        # Ac = MultiHeadAttn(Q=ST, K=SE, V=SE)
        A_c, _ = self.attn_c(query=ST, key=SE, value=SE)  # (B, T, d_model)

        # 公式 (33): CR = MLP(Concat[As, Ac])
        concat_A = torch.cat([A_s, A_c], dim=-1)  # (B, T, 2 * d_model)
